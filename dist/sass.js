var cm6 = (() => {
  ...
  var identifiers2 = new ExternalTokenizer((input, stack) => {
    for (let inside2 = false, dashes = 0, i = 0; ; i++) {
      let { next } = input;
      if (isAlpha2(next) || next == dash2 || next == underscore2 || inside2 && isDigit2(next)) {
        ...
        if (inside2)
          input.acceptToken(next == parenL2 ? callee2 : dashes == 2 && stack.canShift(VariableName2) ? VariableName2 : identifier2);
        break;
      }
    }
  });
  var descendant2 = new ExternalTokenizer((input) => {
    if (space2.includes(input.peek(-1))) {
      let { next } = input;
      if (isAlpha2(next) || next == underscore2 || next == hash2 || next == period2 || next == bracketL2 || next == colon2 && isAlpha2(input.peek(1)) || next == dash2 || next == ampersand)
  ...
  var spec_callee2 = { __proto__: null, lang: 32, "nth-child": 32, "nth-last-child": 32, "nth-of-type": 32, "nth-last-of-type": 32, dir: 32, "host-context": 32, url: 60, "url-prefix": 60, domain: 60, regexp: 60, selector: 138 };
  var spec_AtKeyword2 = { __proto__: null, "@import": 118, "@media": 142, "@charset": 146, "@namespace": 150, "@keyframes": 156, "@supports": 168 };
  var spec_identifier2 = { __proto__: null, not: 132, only: 132 };
  var parser2 = LRParser.deserialize({
    ...
    tokenizers: [descendant2, unitToken2, identifiers2, 1, 2, 3, 4, new LocalTokenGroup("m~RRYZ[z{a~~g~aO#^~~dP!P!Qg~lO#_~~", 28, 105)],
    topRules: { "StyleSheet": [0, 4], "Styles": [1, 86] },
    specialized: [{ term: 100, get: (value) => spec_callee2[value] || -1 }, { term: 58, get: (value) => spec_AtKeyword2[value] || -1 }, { term: 101, get: (value) => spec_identifier2[value] || -1 }],
    tokenPrec: 1200
  });
