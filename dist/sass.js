var cm6 = (() => {
  ...
  function tagHighlighter(tags3, options2) {
  ...
  function changeBlockComment(option, state, ranges = state.selection.ranges) {
    ...
    if (!tokens.every((c) => c))
      return null;
    let comments2 = ranges.map((r, i) => findBlockComment(state, tokens[i], r.from, r.to));
    if (option != 2 && !comments2.every((c) => c)) {
      return { changes: state.changes(ranges.map((range, i) => {
        if (comments2[i])
          return [];
        return [{ from: range.from, insert: tokens[i].open + " " }, { from: range.to, insert: " " + tokens[i].close }];
      })) };
    } else if (option != 1 && comments2.some((c) => c)) {
      let changes = [];
      for (let i = 0, comment2; i < comments2.length; i++)
        if (comment2 = comments2[i]) {
  ...
  var descendantOp2 = 122;
  var Unit2 = 1;
  var identifier2 = 123;
  var callee2 = 124;
  var VariableName2 = 2;
  var queryIdentifier2 = 125;
  ...
  function isHex(ch) {
    return isDigit2(ch) || ch >= 97 && ch <= 102 || ch >= 65 && ch <= 70;
  }
  var identifierTokens = (id2, varName, callee3) => (input, stack) => {
    for (let inside2 = false, dashes = 0, i = 0; ; i++) {
      let { next } = input;
      if (isAlpha2(next) || next == dash2 || next == underscore2 || inside2 && isDigit2(next)) {
        ...
        if (inside2) input.acceptToken(
          dashes == 2 && stack.canShift(VariableName2) ? varName : next == parenL2 ? callee3 : id2
        );
        break;
      }
    }
  };
  var identifiers2 = new ExternalTokenizer(
    identifierTokens(identifier2, VariableName2, callee2)
  );
  var queryIdentifiers = new ExternalTokenizer(
    identifierTokens(queryIdentifier2, queryVariableName, QueryCallee)
  );
  var descendant2 = new ExternalTokenizer((input) => {
    if (space2.includes(input.peek(-1))) {
      let { next } = input;
      if (isAlpha2(next) || next == underscore2 || next == hash2 || next == period2 || next == asterisk2 || next == bracketL2 || next == colon2 && isAlpha2(input.peek(1)) || next == dash2 || next == ampersand)
        input.acceptToken(descendantOp2);
    }
  });
  var unitToken2 = new ExternalTokenizer((input) => {
    if (!space2.includes(input.peek(-1))) {
      let { next } = input;
      if (next == percent2) {
        input.advance();
      ...
      if (isAlpha2(next)) {
        do {
          input.advance();
        } while (isAlpha2(input.next) || isDigit2(input.next));
        input.acceptToken(Unit2);
      }
    }
  });
  var cssHighlighting2 = styleTags({
  ...
  var spec_callee2 = { __proto__: null, lang: 38, "nth-child": 38, "nth-last-child": 38, "nth-of-type": 38, "nth-last-of-type": 38, dir: 38, "host-context": 38, if: 84, url: 124, "url-prefix": 124, domain: 124, regexp: 124 };
  var spec_queryIdentifier2 = { __proto__: null, or: 98, and: 98, not: 106, only: 106, layer: 170 };
  var spec_QueryCallee = { __proto__: null, selector: 112, layer: 166 };
  var spec_AtKeyword2 = { __proto__: null, "@import": 162, "@media": 174, "@charset": 178, "@namespace": 182, "@keyframes": 188, "@supports": 200, "@scope": 204 };
  ...
  var parser2 = LRParser.deserialize({
    ...
    tokenizers: [descendant2, unitToken2, identifiers2, queryIdentifiers, 1, 2, 3, 4, new LocalTokenGroup("m~RRYZ[z{a~~g~aO#v~~dP!P!Qg~lO#w~~", 28, 129)],
    topRules: { "StyleSheet": [0, 6], "Styles": [1, 105] },
    specialized: [{ term: 124, get: (value) => spec_callee2[value] || -1 }, { term: 125, get: (value) => spec_queryIdentifier2[value] || -1 }, { term: 4, get: (value) => spec_QueryCallee[value] || -1 }, { term: 25, get: (value) => spec_AtKeyword2[value] || -1 }, { term: 123, get: (value) => spec_identifier2[value] || -1 }],
    tokenPrec: 1963
  });
  ...
  var defineCSSCompletionSource = (isVariable) => (context) => {
    ...
    if (node.name == "PropertyName" || (isDash || node.name == "TagName") && /^(Block|Styles)$/.test(node.resolve(node.to).name))
      return { from: node.from, options: properties(), validFor: identifier3 };
    if (node.name == "ValueName")
      return { from: node.from, options: values, validFor: identifier3 };
    if (node.name == "PseudoClassName")
      return { from: node.from, options: pseudoClasses, validFor: identifier3 };
    ...
    if (node.name == "TagName") {
      for (let { parent } = node; parent; parent = parent.parent)
        if (parent.name == "Block")
          return { from: node.from, options: properties(), validFor: identifier3 };
      return { from: node.from, options: tags2, validFor: identifier3 };
    }
    if (node.name == "AtKeyword")
      return { from: node.from, options: atRules, validFor: identifier3 };
    if (!context.explicit)
      return null;
    let above = node.resolve(pos), before = above.childBefore(pos);
    if (before && before.name == ":" && above.name == "PseudoClassSelector")
      return { from: pos, options: pseudoClasses, validFor: identifier3 };
    if (before && before.name == ":" && above.name == "Declaration" || above.name == "ArgList")
      return { from: pos, options: values, validFor: identifier3 };
    if (above.name == "Block" || above.name == "Styles")
      return { from: pos, options: properties(), validFor: identifier3 };
    return null;
  };
  ...
  return __toCommonJS(sass_exports);
})();
